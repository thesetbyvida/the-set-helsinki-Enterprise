...
export function PayrollPage(){
  ...
  const byEmp=aggregatePayrollHours(shifts,specialDays);const ot=calculateOvertimeByEmployee(visibleEmployees,overtimeShifts,completedPeriods,overtimeSpecialDays);const adj=new Map<string,number>();for(const a of adjustments)adj.set(a.employee_id,(adj.get(a.employee_id)||0)+Number(a.amount||0));setRows(visibleEmployees.map(e=>calculatePayrollRow(e,byEmp.get(e.id)||emptyTes(),settings,ot.get(e.id),adj.get(e.id)||0)))}catch(e){setError(formatError(e))}finally{setLoading(false)}})()},[restaurantId,period.start,period.end,visibleEmployees,settings]);
  ...
  return <div className="payroll-page">
    ...
    <div className="panel payroll-rates no-print"><div><h3>{t.payrollRates||"Payroll rates"}</h3><p className="muted">Phase 4.4 TES logic: evening 18–24, night 00–06, Sunday/holiday +100% including time supplements, aatto after 15:00 +50%, overtime above 120 h in a 3-week period (first 18 h +50%, then +100%).</p></div><div className="payroll-rate-grid">
      ...
      <div className="payroll-rule-chip"><span>Aatto</span><strong>+50%</strong></div>
      <div className="payroll-rule-chip"><span>Overtime</span><strong>120h → 18h +50%, then +100%</strong></div>
    ...
    <div className="panel payroll-footnote no-print">TES 4.4: monthly reference hourly rate uses Monthly Salary ÷ 159 when no explicit hourly rate is stored. Base h = worked + paid S + VL. Sunday/holiday pays +100% base wage and also +100% of evening/night supplements on overlapping hours. Aatto pays +50% of base wage after 15:00 and +50% of the evening supplement after 18:00; it is not paid when the eve falls on a holiday. Additional work is separated up to 120 h; overtime above 120 h is split into first 18 h at +50% and following hours at +100%. VV is tracked separately.</div>
  </div>
}
